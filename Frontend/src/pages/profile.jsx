    ...
    return (
        <>
            <div className='relative min-h-screen w-full flex justify-center '>
                ...
                <div className=' relative main z-10  border-2 w-[70%]  flex flex-col items-center p-5 gap-5'>
                    <div className='top flex w-full h-80 border-2 px-2 justify-center gap-7 bg-white'>
                        ...
                        <div className=' relative flex border-2 w-[80%] mt-10  z-0 rounded-xl'>
                            ...
                            <div onClick={()=>{setmodalstate("cover")}} className='absolute right-5  top-2 z-20 cursor-pointer' ><EditIcon sx={{ color: "white" }} /></div>
                            <div className='border-2 w-25 rounded-full h-25 absolute bottom-5 left-5' onClick={()=>{setmodalstate("profile")}}><EditIcon sx={{ color: "white" }} className='absolute bottom-1 right-[-5px] cursor-pointer' /></div>
                        </div>
                ...
                {
                    modalstate === "cover" && <Modal closemodal={()=>{setmodalstate(null)}} title="Upload Cover Image">
                        <Imagemodal Data={data} />
                    </Modal>
                }
                {
                    modalstate === "profile" && <Modal closemodal={()=>{setmodalstate(null)}} title="Upload Cover Image">
                        <Imagemodal Data={data} />
                    </Modal>
                }
